...
description: Create and manage API keys for the Feather CX Platform. Pass your key in the x-api-key header or as a Bearer token on every request.
---
...
Feather supports two ways to pass your key on each request. Both are accepted on every endpoint; use whichever fits your stack best.

### API Key Header (Preferred)

Pass the key in the `x-api-key` request header. This is the recommended approach because it is unambiguous and works seamlessly with API gateways and proxies that strip or rewrite `Authorization` headers.
...
Pass the key as a standard `Authorization: Bearer` header. Use this method when you are working with an HTTP client or framework that expects OAuth-style credentials.

<CodeGroup>

```bash Bearer Token
curl https://api-sandbox.featherhq.com/v1/identity/whoami \
  -H 'Authorization: Bearer fth_live_xxxxxxxxxxxxxxxxxxxx'
```
...
```json 401 Authentication Required
{
  ...
  "message": "No valid API key was provided. Pass your key in the x-api-key header or as a Bearer token."
}
```
